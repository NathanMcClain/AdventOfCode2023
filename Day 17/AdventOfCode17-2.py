...
while q:
    ...
    #print('path',curPath)
    if curRow == destination_idx[0] and curCol == destination_idx[1] and curHeatLost < least_heat and 4 <= curNumInDirection <= 10:
    ...
    if curNumInDirection > 10:
        #max of 10
        continue
    if curDirection == -1:
        possibleDirections = [1,2]
    elif 4 <= curNumInDirection < 10:
        possibleDirections = [(curDirection + 1)%4,(curDirection-1+4)%4,curDirection]
    elif curNumInDirection == 10:
        possibleDirections = [(curDirection + 1)%4,(curDirection-1+4)%4]
    else:
        #num in direction of less than 4, only possible direction is the same direction
        possibleDirections = [curDirection]
    ...
    for eachpossibleDirection in possibleDirections:
        numMoves = 4
        if eachpossibleDirection == curDirection:
            numMoves = 1
        newRow = curRow + (next_offset[eachpossibleDirection][0] * numMoves)
        newCol = curCol + (next_offset[eachpossibleDirection][1] * numMoves)
        if 0 <= newRow < len(grid):
            if 0 <= newCol < len(grid[0]):
                newHeatLoss = curHeatLost
                for r in range(1,numMoves+1):
                    newHeatLoss += int(grid[curRow + (next_offset[eachpossibleDirection][0] * r)][curCol + (next_offset[eachpossibleDirection][1] * r)])
                ...
                newNumInDirection = curNumInDirection + 1 if eachpossibleDirection == curDirection else 4
